...
func (r *SegmentationRepository) InsertPackage(items []model.Segmentation) error {
	...
	for _, item := range items {
       ...
       VALUES (:address_sap_id, :adr_segment, :segment_id ) ON CONFLICT (address_sap_id) DO UPDATE adr_segment = excluded.adr_segment, segment_id = excluded.segment_id`, item)
	}
	return tx.Commit()
}
